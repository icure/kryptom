					...
					outBytes.usePinned { pinnedOut ->
						key.rawKey.usePinned { pinnedKey ->
							...
							try {
								...
								totalMoved += dataOutMoved.value.toInt()
								println("Moved $totalMoved (+16 = ${totalMoved + IV_BYTE_LENGTH}), outBytes size ${outBytes.size}")
								if ((totalMoved + IV_BYTE_LENGTH) == outBytes.size) outBytes else outBytes.copyOf(IV_BYTE_LENGTH + totalMoved)
...
//								)
	...
	private fun getCcMode(
		...
		AesAlgorithm.CtrWithPkcs7Padding -> kCCModeCTR
	}
}